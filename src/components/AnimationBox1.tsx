...
export default function AnimationBox1({ children, id = 'box-1' }: Props) {
  ...
  const [started, setStarted] = useState(false); // used to animate "Keep scrolling"
  ...
  useEffect(() => {
    ...
    function setCanvasSize() {
      ...
      // Ensure drawing uses device pixel ratio
    ...
    // draw using preloaded images when possible
    ...
    // slower interpolation factor for smoother/longer playback
    const INTERP = 0.06;
    ...
    const onScroll = () => {
      ...
      // only activate when at least 30% visible
      const ratio = intersectionRef.current ?? 0;
      if (ratio < 0.3) return;

      // compute eased ratio based on intersection ratio
      const eased = easeOutQuad(Math.min(1, Math.max(0, ratio)));
      targetFrame = Math.round((frameCount - 1) * eased);

      // mark started on first down-scroll
  ...
  return (
    <section
      id={id}
      className="w-[45vw] h-[80vh] rounded-xl shadow-lg bg-gradient-to-b from-white to-gray-100 overflow-hidden p-6 relative flex items-center justify-center"
    >
